...
export default function NetshortWatchPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/short/netshort/stream?id=${id}&episode=${episode}`);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f]">
      <div className="max-w-5xl mx-auto px-4 py-6">
        <Link href={`/short/netshort/${id}`} className="text-gray-400 hover:text-white mb-4 inline-block">
          ← Back to Detail
        </Link>
