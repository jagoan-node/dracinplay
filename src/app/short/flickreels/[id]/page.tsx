...
export default function FlickreelsDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/short/flickreels/detail?id=${id}`);
      ...
      <div className="relative -mt-32 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1 min-w-0">
            <span className="bg-indigo-500 text-white text-xs px-3 py-1 rounded-full font-semibold">FlickReels</span>
          ...
          <section className="mt-12">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
              {Array.from({ length: epCount }, (_, i) => i + 1).map((ep) => (
                <Link key={ep} href={`/short/flickreels/watch/${id}/${ep}`} className="flex items-center justify-center py-3 px-3 rounded-lg text-sm font-medium bg-[#1a1a2e] text-gray-400 hover:text-white hover:bg-[#252540] border border-white/10 transition-all">
                  {ep}
                </Link>
              ))}
            </div>
